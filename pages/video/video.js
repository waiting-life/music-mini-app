...
Page({
  ...
  // 点击切换导航的回调
  changeItem(e) {
    //1. 通过id传参的时候，如果传的是number会自动转换成string
  ...
  handleEnded(e) {
    // console.log('播放结束')
    const {videoUpdateTime} = this.data
    let curIndex = videoUpdateTime.findIndex(item => item.vid === e.currentTarget.id)
    videoUpdateTime.splice(curIndex, 1)
    this.setData({
      videoUpdateTime
    })
  },
